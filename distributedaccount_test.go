...
func TestCreateDistributedAccount(t *testing.T) {
	...
	wallet, err := dirk.OpenWallet(ctx, "Distributed wallet", credentials, endpoints)
	require.NoError(t, err)
	...
	_, err = accountCreator.CreateDistributedAccount(context.Background(), accountName, 3, 2, []byte("secret"))
	require.NoError(t, err)
